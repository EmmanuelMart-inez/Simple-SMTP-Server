...
$|            = 1;
my($p_name)   = $0 =~ m|/?([^/]+)$|;
my $p_version = build_version("20190914.0", '$Id$');
...
sub get_option_struct {
	@G::raw_option_data = (
		...
		# req tls if avail
		{ opts    => ['tlsos', 'tls-optional-strict'],                     suffix => '',
		...
		# A file to attach
		{ opts    => ['attach-name','attach-type','attach','attach-body'], suffix => ':s',
...
sub fetch_args {
	...
	for (my $i = 0; $i < scalar(@ARGV); $i++) {
		...
		# if we're referencing an option which was previously marked for removal, remove the removal flag
		if ($ARGV[$i] =~ /^-?-(.*)$/) {
			my $opt_name = $1;
			foreach my $e (@$l) {
		...
		if ($ARGV[$i] =~ /^-?-h(?:eader)?-([^:]+):?$/) {
			...
			# allow --no-OPTION to remove any previous OPTION settings
			...
			# all of the --attach* options end up in the same array because the order we receive them in
			# is important.  But we only get the value, so flag them so we will be able to tell which
			# argument belongs with which option later

			my $opt = uc($1);
			if ($ARGV[$i] =~ /^-?-attach-name$/ && $ARGV[$i+1] =~ /^-/) {
				...
				splice(@ARGV, $i+1, 0, 'SWAKS_ATTACH-NAME:');
			}
			else {
				$ARGV[$i+1] = 'SWAKS_' . $opt . ':' . $ARGV[$i+1];
			}
		}
	}
...
sub process_args {
	...
	if (defined($o->{show_time_lapse})) {
		if (length($o->{show_time_lapse}) && $o->{show_time_lapse} !~ /^i/i) {
			ptrans(12, "Unknown argument '$o->{show_time_lapse}' to option show-time-lapse, exiting");
			exit(1);
		}
		if (avail("hires_timing") && $o->{show_time_lapse} !~ /^i/i) {
	...
	if ($G::link{type} eq 'socket-inet') {
		...
		if (defined($o->{copy_routing})) {
			if ($o->{copy_routing}) {
				$n{copy_routing}   = $o->{copy_routing}; # only used in --dump output
				$G::link{server} ||= get_server($o->{copy_routing});
			}
			else {
				ptrans(12, "Option --copy-routing requires an argument");
				exit 1;
			}
		}
		else {
	...
	if (defined($o->{attach_822}) && ref($o->{attach_822}) eq 'ARRAY' && scalar(@{$o->{attach_822}})) {
		...
		while (defined(my $t = shift(@{$o->{attach_822}}))) {
			if ($t =~ /^SWAKS_ATTACH-TYPE:(.*)$/) {
				$mime_type = $1;
			} elsif ($t =~ /^SWAKS_ATTACH-NAME:(.*)$/) {
				$next_name = $1;
			} elsif ($t =~ /^SWAKS_ATTACH-BODY:(.*)$/) {
				if ($mime_type eq '%SWAKS_DEFAULT_MIMETYTPE%') {
					$mime_type = 'text/plain';
				}
				push(@{$parts{body}}, { body => $1, type => $mime_type });
			...
			} elsif ($t =~ /^SWAKS_ATTACH:(.*)$/) {
				push(@{$parts{rest}}, { body => $1, type => $mime_type });
				if (defined($next_name)) {
					$parts{rest}[-1]{name} = $next_name;
					$next_name             = undef();
				}
			} else {
				ptrans(12, "Error processing attach args, unknown type when processing $t");
				exit(1);
			}
		}
		...
		# now loop through and reconcile stdin/file contents
		foreach my $type ('body', 'rest') {
			foreach my $part (@{$parts{$type}}) {
				if ($part->{body} eq '-') {
					if ($stdin) {
						$part->{body} = $stdin;
					} else {
						$part->{body} = join('', <STDIN>);
						$stdin        = $part->{body};
					}
				} elsif (open(I, "<$part->{body}")) {
					if (!exists($part->{name}) && $type ne 'body') {
						($part->{name}) = $part->{body} =~ m|/?([^/]+)$|;
					}
					$part->{body} = join('', <I>);
					close(I);
				}
	...
	if ($o->{data_on_stdin}) {
		...
		# if it doesn't have any newlines assume it's a file. Fail if we can't open.
		if (!open(F, "<$n{data}")) {
			ptrans(12, "data option appears to be a file but is not openable: $n{data} ($!)");
			exit(1);
		}
		$n{data} = join('', <F>);
		close(F);
...
Specify which TCP port on the target is to be used, or prompt if no argument is listed.  The argument can be a service name (as retrieved by getservbyname(3)) or a port number.  The default port is smtp/25 unless influenced by the --protocol or --tls-on-connect options.
...
The option is similar to --quit-after, but instead of trying to cleanly shut down the session it simply terminates the session.  This option accepts the same stop-points as --quit-after and additionally accepts DATA and DOT, detailed below.
...
This option is similar to --drop-after, but instead of dropping the connection after reading a response to the stop-point, it drops the connection immediately after sending stop-point.  It accepts the same stop-points as --drop-after.
...
If the argument does not contain any literal (0x0a) or representative (0x5c, 0x6e or %NEWLINE%) newline characters, it will be treated as a filename.  If the file is open-able, the contents of the file will be used as the data portion.  If the file cannot be opened, Swaks will error and exit.
...
If the message is forced to MIME format (see --attach) "--body 'body text'" is the same as "--attach-type text/plain --attach-body 'body text'".  See --attach-body for details on creating a multipart/alternative body.
...
If the contents of the attachment are provided via a file name, the MIME encoding will include that file name.  See --attach-name for more detail on file naming.
...
Environment variable prefix used to specify Swaks options from environment variables.  See OPTION PROCESSING -> ENVIRONMENT VARIABLES above.
